...
def get_weather_icon_with_time(code, current_time, sunrise_time=None, sunset_time=None):
    """Return emoji and description for WMO weather code,
       using day/night for clear, mainly clear, and partly cloudy."""
    # Base mapping for all codes
    mapping = {
        0: ("☀️", "Clear sky"),
        1: ("🌤️", "Mainly clear"),
        2: ("⛅", "Partly cloudy"),
    ...
    base_icon, condition = mapping.get(code, ("🌈", "Unknown"))

    # For clear, mainly clear, and partly cloudy – replace with night versions
    if code in (0, 1, 2) and sunrise_time and sunset_time:
        # Compare naive datetimes (all UTC)
        if current_time < sunrise_time or current_time > sunset_time:
            if code == 0:
                return ("🌙", "Clear night")
            elif code == 1:
                return ("🌤️", "Mainly clear night")
            elif code == 2:
                return ("☁️", "Partly cloudy night")
    return base_icon, condition


def fetch_weather_data(lat, lon):
    """Fetch current, hourly, and daily weather from Open-Meteo (UTC timezone)."""
    params = {
        "latitude": lat,
        "longitude": lon,
...
def weather():
    ...
    try:
        ...
        # Parse current time (UTC string)
        current_time = datetime.fromisoformat(current_time_str) if current_time_str else datetime.utcnow()
        ...
        # Get sunrise/sunset for today (first day in daily)
        daily = data.get("daily", {})
        sunrise_time = sunset_time = None
        if "sunrise" in daily and daily["sunrise"]:
            try:
                sunrise_time = datetime.fromisoformat(daily["sunrise"][0])
                sunset_time = datetime.fromisoformat(daily["sunset"][0])
            except:
                pass

        # Choose icon with day/night awareness
        icon_emoji, condition = get_weather_icon_with_time(
            weather_code, current_time, sunrise_time, sunset_time
        )
